//
//  AgencyRequestViewController.swift
...
class AgencyRequestViewController: UIViewController {
   ...
   @objc func fetchData(){
        let api = APIWrapper()
        let response = api.getMethodCall(controllerName: "User", actionName: "Agencyaccountverification")
        if response.ResponseCode == 200 {
            self.refreshControl.endRefreshing()

            allvendors = try! JSONDecoder().decode([LoginModel].self, from: response.ResponseData!)
            print("All Agency :\(allvendors.count)")
...
extension AgencyRequestViewController :UITableViewDataSource, UITableViewDelegate{
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = tblranking.dequeueReusableCell(withIdentifier: "cell", for: indexPath) as! AgencyrequestTableViewCell
    ...
    func Update(id:Int) {
        ...
        let endpoint = "User/Agencyrequest?id=\(id)"
